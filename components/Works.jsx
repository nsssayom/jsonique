...
export default function Works({ data }) {
  ...
  return (
    <div className="lg:space-y-20 h-full flex flex-col w-full pt-16 lg:pt-0">
      <div className="grid lg:grid-cols-2 lg:gap-20 grow">
          ...
          <div className="flex flex-col leading-none w-full items-center justify-center space-y-8">
            ...
            <div className="w-full flex flex-col lg:hidden">
              {data?.content?.experiences?.map((experience, index) => (
                <div key={index} className="mb-4">
                  <div
                    className={`text-base font-normal flex items-center justify-between p-3 px-5 lg:rounded-3xl cursor-pointer space-x-2 ${
                      index === selectedWork
                        ? "bg-zinc-200 text-black rounded-t-md"
                        : "bg-zinc-100 text-black rounded-md"
                    }`}
            ...
            <div className="space-y-4 w-full lg:flex flex-col items-center justify-center hidden">
              {data?.content?.experiences?.map((experience, index) => (
                <div
                  key={index}
                  className={`text-base font-normal flex items-center justify-center p-3 w-full rounded-3xl shadow-md cursor-pointer ${
                    index === selectedWork
        ...
        <div className="col-span-1 lg:flex flex-col items-center justify-center space-y-12 hidden pt-10">
      ...
      <div className="lg:grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-2 lg:gap-4 text-center hidden">
        {uniqueSkills?.map((skill, index) => (
          <span
            key={index}
            className={`p-1 lg:p-1.5 rounded-md shadow-md ${
              skill.skillIndex.includes(selectedWork)
                ? "bg-zinc-200 text-black"
                : "bg-zinc-100 text-black"
            }`}
          >
